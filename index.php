                ...
                if ( $latest_travel->have_posts() ) {
                    while ( $latest_travel->have_posts() ) {
                        ...
                        if ( !empty($data["cover-photo"]) ) {
                            ...
                            echo ($current_post_type == 'travel_locations') ? $data['location-en'] : $data['location-post-location-en'];
                            echo '</p><h5>' ;
                            echo ($current_post_type == 'travel_locations') ? $data['location-lang'] : $data['experience'];
                            echo '</h5><div class="button"><p>';
                            echo ($current_post_type == 'travel_locations') ? 'Explore ' . $data['city'] : 'More About ' . $data['experience'];
                            echo '</p></div></a>';
                        } 
                    }
                }
...
<?php
                $current_date = current_time('Ymd'); 
                $args = array('post_type' => 'travel_locations');
                $query = new WP_Query( $args );
                echo $query->found_posts;
?> 
                </p>
