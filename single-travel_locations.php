<?php
    ...
    */
    get_header();
    page_style("single-travel-locations");
    page_script("single-travel-locations");
    global $post;
    if ( have_posts() ) :
...
<?php
                        ...
                        foreach ($location_keys as $header => $field) {
                            ...
                            if (is_array($field)) {
                                ...
                                $value = implode(" ", $value);
                                /*if ([count(explode("-", $field[0])) - 1] == "post") {
                                    $field[$i]->the_post();
                                    for ($j = 0; $j < count($field); $j++) {
                                        //$value[] = get_field($field[$i]);
                                    }
