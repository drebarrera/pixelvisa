...
document.addEventListener("DOMContentLoaded", function() {
    ...
    for (let i = 0; i < marker_data.length; i++) {
        ...
        if ( !(marker_data[i][3] in markers_placed) ) {
            ...
            marker.bindPopup("<b>" + marker_datum[1] + " " + marker_datum[2] + "</b>", { autoPan: false }).openPopup();
    ...
    for (let datum in panel_data) {
        ...
        const location_country_p = document.createElement("p"); // Flag & Country subheader
        location_country_p.textContent = panel_data[datum].flag + " "  + panel_data[datum].country;
        location_country_p.className = "location-country-p";
        location_entry.appendChild(location_country_p);
        for (let i = 0; i < panel_data[datum].entry.length; i++) {
            ...
            location_entry.onclick = function(e) {
                map.flyTo(panel_data[datum].coordinates, 16);
                var isLocation = true;
                var target = e.target;
