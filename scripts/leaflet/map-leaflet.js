...
document.addEventListener("DOMContentLoaded", function() {
    ...
    for (let datum in panel_data) {
        ...
        cities[datum.toLowerCase()] = [location_entry.dataset.markerid, datum];
        ...
        if (!countries.hasOwnProperty(panel_data[datum].country.toLowerCase())) countries[panel_data[datum].country.toLowerCase()] = [[location_entry.dataset.markerid], panel_data[datum].country];
        else countries[panel_data[datum].country.toLowerCase()][0].push(location_entry.dataset.markerid);
    ...
    map.on('zoomend', function () { 
        ...
        if (map.getZoom() <= 5) {
            ...
            for (let i = 0; i < paths.length; i++) {
                if (paths[i].dataset.pathcolor == undefined) {
                    var strokeColor = window.getComputedStyle(paths[i]).getPropertyValue("stroke");
                    if (strokeColor == undefined) paths[i].dataset.pathcolor = "";
                    else paths[i].dataset.pathcolor = strokeColor;
                }
